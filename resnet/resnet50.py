...
def dense(inputs, out_dimension, use_biase=False,name=None):
    return slim.fully_connected(inputs=inputs,num_outputs=out_dimension,activation_fn=None,scope=name)
    # return tf.layers.dense(
    #     inputs=inputs, units=out_dimension, use_bias=use_biase,
    #     name=name,kernel_initializer=tf.variance_scaling_initializer())


def conv2d(inputs, out_channal, kernel_size, strides, use_bias=False, name=None):
    """
    卷积
    :param inputs: [批，高，宽，通道数]
    ...
    return slim.conv2d(inputs=inputs, num_outputs=out_channal,
                       kernel_size=kernel_size, padding=padding,activation_fn=None, scope=name, stride=strides)
...
def conv2d_transpose(inputs, out_channal, kernel_size, strides, name=None):
...
class Model(object):
    ...
    def __call__(self, inputs, training):
        ...
        return layer_values, resolution
